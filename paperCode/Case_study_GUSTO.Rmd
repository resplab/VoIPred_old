...
library(here)
load_file=here("data","2021.10.07.RDS")
...
# num_runs <- data.frame(n=10,sample_size=c(500,1000,2000,4000,8000,16000,32000,40830))
# library(foreach)
# library(doMC)
...
# # save each result
# tmp_results <- foreach(i=1:nrow(num_runs), .combine=cbind) %dopar% {
#   voi_by_sample_size_custom(n_sim=num_runs$n[i],sample_sizes =num_runs$sample_size[i])
# }

# process the results
sim_files <- list.files(here('simulation_result'))

sim_results <- lapply(sim_files,function(x){
  read_rds(here('simulation_result',x))
}) %>% 
  do.call(rbind,.)
...
voi_relative <- sim_results %>%
  ...
  group_by(sample_size) %>%
  summarise_at(vars(contains("voi_r")),median)

require(ggplot2)
require(tidyr)

df_voi_th <- data.frame(sample=voi_absolute[,1])
df_voi_th <- cbind(df_voi_th,voi_absolute[,grep("voi_th",colnames(voi_absolute))])
df_voi_r <- data.frame(sample=voi_relative[,1])
df_voi_r <- cbind(df_voi_r,voi_relative[,grep("voi_r",colnames(voi_relative))])
colnames(df_voi_th)[-1] <-   settings$custom_th
colnames(df_voi_r)[-1] <- colnames(df_voi_th)[-1]
...
ggplot(data=df_voi_th %>% 
         mutate(log_sample_size = log(sample_size)),aes(x=sample_size,y=EVPI,color=Threshold)) +
